...
app.post('/api/riskItems', upload.single('planFiles'), (req: Request, res: Response) => {
...
app.get('/api/riskItems', (req: Request, res: Response) => {
  ...
  db.all(fetchQuery, [itemsPerPage, offset], (err, rows: Array<RiskItem>) => {
    if (err) {
...
app.get('/api/lastRiskItems', (req: Request, res: Response) => {
  ...
  db.all(fetchQuery, [numberOfLastItems], (err, rows: Array<RiskItem>) => {
    if (err) {
      console.error('Error fetching risk items:', err);
      return res.status(500).json({ error: 'Failed to fetch risk items from database' });
    }
...
app.put('/api/riskItems/:id', (req: Request, res: Response) => {
...
app.get('/api/appliedChecklists', (req: Request, res: Response) => {
  const page = parseInt(req.query.page as string) || 1; // Default to page 1
  ...
  db.all(fetchQuery, [itemsPerPage, offset], (err, rows: Array<AppliedChecklist>) => {
    if (err) {
...
app.post('/api/appliedChecklists', (req: Request, res: Response) => {
  const newAppliedChecklist = req.body as unknown as AppliedChecklist;
  ...
  if (newAppliedChecklist) {
    ...
    db.run(
      insertQuery,
      [newAppliedChecklist.title || '', newAppliedChecklist.dateApplied || ''],
      function (err) {
        if (err) {
...
app.get('/api/chartData', (req: Request, res: Response) => {
  const fetchQuery = `
    SELECT planApproval
    FROM risk_items
  `;

  db.all(fetchQuery, (err, rows: Array<{ planApproval: string}>) => {
    if (err) {
...
app.get('/api/downloadPlanFile/:id', async (req: Request, res: Response) => {
  ...
  db.get(fetchQuery, [riskId], async (err, row) => {
    ...
    const riskItem = row as { planFiles: Blob | string, planFilesName: string };
    ...
    if (typeof riskItem.planFiles === 'string') {
      ...
      const planFilesData = riskItem.planFiles as Blob;
...
app.post('/api/approveRiskItem/:riskId', (req: Request, res: Response) => {
...
app.post('/api/rejectRiskItem/:riskId', (req: Request, res: Response) => {
  const { riskId } = req.params;
